...
nextStepBtn.addEventListener('click', () => switchStep(2));
...
function switchStep(step) {
    ...
    if (step === 2) {
        ...
        step1Indicator.classList.replace('step-active', 'step-inactive'); step2Indicator.classList.replace('step-inactive', 'step-active');
    } else {
        document.getElementById('step-2').style.display = 'none'; document.getElementById('step-1').style.display = 'block';
        step2Indicator.classList.replace('step-active', 'step-inactive'); step1Indicator.classList.replace('step-inactive', 'step-active');
    }
    window.scrollTo(0, 0);
}
...
async function handleSubmission() {
     ...
     try {
        ...
        switchStep(1);
         ...
         document.getElementById('error-message').textContent = "Could not submit your request. Please check your connection and try again.";
         document.getElementById('error-modal').classList.remove('hidden');
     } finally {
         submitBtn.disabled = false; submitBtn.textContent = 'Confirm & Send Request';
     }
}
...
if(window.location.hash === '#admin') {
    document.getElementById('quote-generator').style.display = 'none';
     if(sessionStorage.getItem('isAdmin') === 'true') showAdminDashboard();
     else document.getElementById('admin-login').style.display = 'block';
}

loginForm.addEventListener('submit', (e) => {
    e.preventDefault();
    if(document.getElementById('admin-password').value === ADMIN_PASS) { sessionStorage.setItem('isAdmin', 'true'); showAdminDashboard(); } 
...
function loadRequests() {
    ...
    if(!db) { list.innerHTML = `<p class="text-red-500">Database not connected.</p>`; return; };
    db.collection('rentalRequests').orderBy('createdAt', 'desc').onSnapshot(snapshot => {
        if(snapshot.empty) { list.innerHTML = `<p class="text-gray-500">No requests found.</p>`; return; }
    ...
    }, error => { console.error("Error fetching requests:", error); list.innerHTML = `<p class="text-red-500">Could not load requests.</p>`; });
}

function getStatusClass(s) { return s==='approved' ? 'bg-green-100 text-green-800' : s==='denied' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'; }
...
function renderRequest(id, data) {
     ...
     return `
        <div class="border rounded-lg p-4 transition hover:shadow-md text-sm">
             <div class="flex justify-between items-start">
                 <div>
                    <p class="font-semibold text-lg font-sans">${data.eventTitle || 'No Title'} <span class="text-sm font-normal text-gray-500">- by ${data.name}</span></p>
                     ...
                     <span class="inline-block mt-2 px-2 py-1 text-xs font-semibold rounded-full ${getStatusClass(data.status)} font-sans">${data.status}</span>
                 </div>
                 <div class="text-right">
                     <p class="text-xl font-bold text-black font-sans">$${data.quote.total.toFixed(2)}</p>
                 </div>
             </div>
...
async function handleStatusUpdate(e) {
    ...
    } catch (error) { console.error("Error updating status: ", error); e.target.textContent = `Error`; }
}
